...
export default async function ProductDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const detailBenefits = td.raw("benefits") as Array<[string, string]>;
  ...
  const chips = [
  [td("chips.minOrder"), product.specs.minOrder ?? td("negotiable"), "⚖"],
  [td("chips.exportSuitable"), td("yes"), "◎"],
  [td("chips.packagingType"), product.specs.packagingType ?? td("custom"), "▣"],
  [td("chips.opennessType"), product.specs.opennessType ?? td("byOrder"), "◡"],
  [td("chips.sizeOz"), product.specs.sizeOz ?? td("byOrder"), "↔"],
];
        ...
        <div className="mx-auto w-full max-w-[1440px] px-5 sm:px-7 lg:px-10">
          ...
          <div className="mt-5 grid gap-9 lg:grid-cols-[1.12fr_.88fr] lg:items-start">
            ...
            <div className="lg:pt-3">
              ...
              <h1 className="mt-3 text-3xl font-black leading-tight text-forest-900 sm:text-4xl">{product.title} {td("exportSuffix")}</h1>
              <p className="mt-2 text-sm font-bold text-gold-700">{td("tagline")}</p>
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <PriceInquiryButton href={whatsappHref} label={td("priceInquiry")} pendingNotice={tCommon("pending")} variant="primary" className="flex-1" />
                <DownloadCatalogButton href={settings.catalogUrl} label={td("downloadSpec")} pendingLabel={tCommon("pending")} variant="secondary" className="flex-1" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto w-full max-w-[1440px] space-y-7 px-5 sm:px-7 lg:px-10">
          <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr_1.1fr]">
            <section className="rounded-3xl border border-line-200 bg-white p-5 shadow-sm sm:p-6">
              <SectionTitle>{td("technicalSpecs")}</SectionTitle>
              <div className="mt-5 grid gap-5 sm:grid-cols-[180px_1fr]">
                <div className="relative min-h-48 overflow-hidden rounded-2xl bg-cream-100"><Image src="/images/redesign/tech-pistachio.webp" alt={td("premiumAlt")} fill sizes="190px" className="object-cover" /></div>
                <dl className="divide-y divide-line-200">
                  ...
                  <div className="flex items-center justify-between gap-4 py-2.5 text-xs"><dt className="font-bold text-forest-800">{td("shelfLifeLabel")}</dt><dd className="text-left text-ink-600">{td("shelfLifeValue")}</dd></div>
                </dl>
              </div>
            </section>

            <section className="rounded-3xl border border-line-200 bg-white p-5 shadow-sm sm:p-6">
              <SectionTitle>{td("usagesTarget")}</SectionTitle>
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-2 text-xs text-ink-600">{product.targetMarkets.map((market) => (<span key={market} className="rounded-full border px-3 py-1.5">{market}</span>))}</div>
            ...
            <section className="rounded-3xl border border-line-200 bg-white p-5 shadow-sm sm:p-6">
              <SectionTitle>{td("relatedProducts")}</SectionTitle>
              <div className="mt-5 grid grid-cols-2 gap-3">
                {relatedProducts.map((related) => (
                  <Link key={related.slug} href={`/products/${related.slug}`} className="group rounded-2xl border border-line-200 p-3 text-center transition hover:border-gold-500/50">
                    ...
                    <span className="mt-2 inline-block text-[10px] font-bold text-gold-700">{td("viewProduct")}</span>
                  </Link>
                ))}
              </div>
            </section>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.4fr_.6fr]">
            <section className="relative overflow-hidden rounded-3xl bg-forest-900 text-cream-50">
              <div className="grid min-h-48 items-center gap-5 p-6 sm:grid-cols-[260px_repeat(3,1fr)]">
                <div className="relative min-h-40"><Image src="/images/redesign/benefit-pistachio.webp" alt={td("benefitAlt")} fill sizes="280px" className="object-contain" /></div>
                {detailBenefits.map(([title, body]) => <div key={title} className="border-gold-500/25 text-center sm:border-r sm:px-4"><span className="text-2xl text-gold-500">✦</span><b className="mt-2 block text-sm">{title}</b><p className="mt-2 text-xs leading-6 text-cream-100/65">{body}</p></div>)}
              </div>
            </section>
            <section className="rounded-3xl border border-line-200 bg-white p-6 shadow-sm">
              <SectionTitle>{td("buyingGuide", { title: product.title })}</SectionTitle>
              <p className="mt-5 text-sm leading-8 text-ink-700">{td("buyingGuideBody")}</p>
              {relatedArticles[0] ? <Link href={`/knowledge-base/${relatedArticles[0].slug}`} className="mt-5 inline-flex rounded-full border border-forest-900 px-5 py-2.5 text-xs font-bold text-forest-900 hover:bg-forest-900 hover:text-white">{td("readFullArticle")}</Link> : null}
            </section>
          </div>
